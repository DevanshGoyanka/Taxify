    ...
    {entries.map((entry, index) => {
      ...
      return <div key={entry.id} style={{ padding: 16, marginBottom: 24, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 6 }}>
        ...
        {(isItr2 || entry.propertyType !== 'SELF_OCCUPIED') && <Section title="Rent details"><div style={gridStyle}>
          <Money label="Gross rent received/receivable or lettable value *" required value={entry.annualLettingValue} onValue={(value) => patch(index, { annualLettingValue: Number(value) })} />
        ...
        <Rows title="Tenant details (optional)" add={() => patch(index, { tenantDetails: [...entry.tenantDetails, { tenantSNo: entry.tenantDetails.length + 1, name: '', pan: '', aadhaar: '', panOrTan: '' }] })}>
          <div style={{ marginBottom: 10, fontSize: 11, color: 'var(--text-muted)' }}>
            PAN/Aadhaar of tenant is mandatory if tax is deducted under section 194-IB. TAN of tenant is mandatory if tax is deducted under section 194-I.
          </div>
          {entry.tenantDetails.map((tenant: TenantDetail, row) => <div key={row} style={gridStyle}><Field label={`Serial ${row + 1} — Name *`} type="text" value={tenant.name} required maxLength={125} onValue={(value) => patchNested(index, 'tenantDetails', row, { name: String(value) })} /><Field label="PAN" type="text" value={tenant.pan} pattern={PAN_PATTERN} maxLength={10} onValue={(value) => patchNested(index, 'tenantDetails', row, { pan: String(value).toUpperCase() })} /><Field label="Aadhaar" type="text" value={tenant.aadhaar} pattern="[0-9]{12}" inputMode="numeric" maxLength={12} onValue={(value) => patchNested(index, 'tenantDetails', row, { aadhaar: String(value) })} /><Field label="PAN / TAN (if TDS credit claimed)" type="text" value={tenant.panOrTan} pattern={PAN_TAN_PATTERN} maxLength={10} onValue={(value) => patchNested(index, 'tenantDetails', row, { panOrTan: String(value).toUpperCase() })} /><Remove onClick={() => removeNested(index, 'tenantDetails', row)} /></div>)}
        </Rows>
