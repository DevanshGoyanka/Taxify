...
export default function ExemptIncomeWorkspace({ form, schedule, onChange, disabled = false }: ExemptIncomeWorkspaceProps): React.JSX.Element {
  ...
  const addEntry = (): void => patch({ otherExemptIncome: [...schedule.otherExemptIncome, { id: id('ei'), category: 'SRPC', subCategory: allowedSubcategories.find((option) => option.value === '10(11)')?.value ?? allowedSubcategories[0]?.value ?? 'Incmexmptnotification', sectionDetails: '', natureOfIncome: '', evidenceReference: '', description: '', grossAmount: 0 }] });
  ...
  if (isItr1) return <div>
    ...
    {schedule.otherExemptIncome.map((entry, index) => <div key={entry.id} style={styles.panel}>
      ...
      <div style={{ marginTop: 16, ...styles.grid }}><Field label="Nature of exempt receipt *" type="text" maxLength={125} value={entry.natureOfIncome} disabled={disabled} onChange={(value) => updateEntry(entry.id, { natureOfIncome: value })} /><Field label="Section / source details" type="text" maxLength={125} value={entry.sectionDetails} disabled={disabled} onChange={(value) => updateEntry(entry.id, { sectionDetails: value })} /><Field label="Evidence reference" type="text" maxLength={125} value={entry.evidenceReference} disabled={disabled} onChange={(value) => updateEntry(entry.id, { evidenceReference: value })} /><Field label="Description / source *" type="text" maxLength={125} value={entry.description} disabled={disabled} onChange={(value) => updateEntry(entry.id, { description: value })} /></div>
    </div>)}
