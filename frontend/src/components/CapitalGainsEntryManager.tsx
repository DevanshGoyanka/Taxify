...
export function CapitalGainsEntryManager({ data: incoming, entries = [], onChange, selectedForm, summary, issues = [] }: Props): React.ReactElement {
  ...
  return <div>
    ...
    {simple && <div style={{ marginBottom: 16, padding: '14px 16px', background: '#fdecec', border: '1px solid #eb6767', borderRadius: 8, fontSize: 13, color: '#eb6767' }}><strong style={{ fontSize: 14, display: 'block', marginBottom: 4 }}>⚠ Switch to ITR-2 or ITR-3 to report these gains</strong><div>The selected form (ITR-1/ITR-4) only permits the simplified section 112A schedule. The full Schedule CG sections below are locked because they cannot be filed under this form. Switch to ITR-2 or ITR-3 using the form selector at the top of the page to unlock and report these transactions.</div></div>}
    {allIssues.length > 0 && <div style={{ padding: 12, marginBottom: 16, color: 'var(--danger)', background: '#fdecec', borderRadius: 6 }}><strong>Review required</strong><ul>{allIssues.map((issue, index) => <li key={index}>{String(issue.message || issue.code || 'Capital-gains issue')}</li>)}</ul></div>}
...
function ApplicabilityBadge({ form, permitted }: { form: string; permitted: boolean }): React.ReactElement | null {
  ...
  return <div style={{ marginBottom: 12, padding: '8px 12px', background: '#fdecec', border: '1px solid #fcd34d', borderRadius: 6, fontSize: 12, color: '#92400e' }}><strong>Not reportable under {form === 'ITR1' ? 'ITR-1' : form === 'ITR2' ? 'ITR-2' : form === 'ITR3' ? 'ITR-3' : 'ITR-4'}.</strong> Entries below are preserved but will not be filed. Switch to {target} to report these gains.</div>;
}
