...
export function CapitalGainsEntryManager({ data: incoming, entries = [], onChange, selectedForm, summary, issues = [] }: Props): React.ReactElement {
  ...
  // display the engine's computation.  These are READ-ONLY overlays — the
  // user edits the underlying row, and the readouts are re-derived.
  ...
  const overlayComputedReadouts = (rows: JsonRow[]): JsonRow[] => rows.map((row, index) => {
  ...
  return <div>
    ...
    {simple && <CollapsibleWarning title="⚠ Switch to ITR-2 or ITR-3 to report these gains">The selected form (ITR-1/ITR-4) only permits the simplified section 112A schedule. The full Schedule CG sections below are locked because they cannot be filed under this form. Switch to ITR-2 or ITR-3 using the form selector at the top of the page to unlock and report these transactions.</CollapsibleWarning>}
    {allIssues.length > 0 && <CollapsibleWarning title="Review required"><ul style={{ margin: '6px 0 0', paddingLeft: 20 }}>{allIssues.map((issue, index) => <li key={index}>{String(issue.message || issue.code || 'Capital-gains issue')}</li>)}</ul></CollapsibleWarning>}
    ...
    <ApplicabilityBadge form={normalizedForm} permitted={!simple} />
    <RowSection title="Schedule 112A scrip details" disabled={simple} rows={schedule112AForDisplay} fields={SCRIP_FIELDS} onChange={setSchedule112ARows} />
    <ScheduleTotals title="Schedule 112A totals" rows={schedule112AForDisplay} />
...
function ApplicabilityBadge({ form, permitted }: { form: string; permitted: boolean }): React.ReactElement | null {
  ...
  return <CollapsibleWarning title={`Not reportable under ${form === 'ITR1' ? 'ITR-1' : form === 'ITR2' ? 'ITR-2' : form === 'ITR3' ? 'ITR-3' : 'ITR-4'}.`}>Entries below are preserved but will not be filed. Switch to {target} to report these gains.</CollapsibleWarning>;
}
