...
export function ITR3PartAOIEditor({ draft, taxResult, onChange }: Props): ReactElement {
  ...
  const inputValue = (key: string, group: string): number => {
    const value = (part as unknown as Record<string, unknown>)[group];
    return typeof value === 'object' && value !== null ? amount((value as Record<string, unknown>)[key]) : amount(value);
  };
  return <section aria-label="Part A-OI" style={{ display: 'grid', gap: 16 }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(220px, 1fr))', gap: 12 }}>
      {ITR3_PART_A_OI_FIELDS.map((field) => <label key={field.path} style={{ display: 'grid', gap: 5, fontSize: 12 }}><span>{field.label}</span><input aria-label={field.label} type="number" min={0} step={1} value={inputValue(field.key, field.group)} onChange={(event) => onChange(updateItr3PartAOI(workspace, field.key, event.target.value))} /><small style={{ color: 'var(--text-secondary)' }}>{field.path}</small></label>)}
    </div>
