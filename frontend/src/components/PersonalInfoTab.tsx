...
export function PersonalInfoTab({ draft, itrForm, onChange, onBanksChange, onRegimeChange }: PersonalInfoTabProps): React.JSX.Element {
  ...
  return <div>
    ...
    </div>}{itrForm === 'ITR-2' && verification.capacity === 'KARTA' && <div style={{ ...GRID_STYLE, marginTop: 16 }}>
      <Field label="Karta's PAN" value={filing.kartaPan} onChange={(value) => updateFiling({ kartaPan: value.toUpperCase().slice(0, 10) })} required pattern={PAN_PATTERN} maxLength={10} help="The Karta's own individual PAN — the HUF's own PAN cannot be used for the Verification declaration." />
