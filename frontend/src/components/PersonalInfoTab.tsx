...
export function PersonalInfoTab({ draft, itrForm, onChange, onBanksChange, onRegimeChange }: PersonalInfoTabProps): React.JSX.Element {
  ...
  return <div>
    ...
    <div style={CARD_STYLE}><h4 style={{ marginTop: 0, fontSize: 14 }}>Filing status</h4><div style={GRID_STYLE}>
      <SelectField label="Return filed under section" value={filing.filingSection} onChange={(value) => updateFiling({ filingSection: value as Filing['filingSection'], returnType: value === '139(5)' ? 'REVISED' : 'ORIGINAL' })} required><option value="139(1)" disabled={dueDatePassed}>139(1) — On or before due date{dueDatePassed ? ' (due date passed)' : ''}</option><option value="139(4)" disabled={!dueDatePassed}>139(4) — Belated return{!dueDatePassed ? ' (due date not passed)' : ''}</option><option value="142(1)">142(1) — Notice</option><option value="148">148 — Reassessment notice</option><option value="153C">153C — Notice</option><option value="139(5)">139(5) — Revised return</option><option value="139(9)">139(9) — Defective return</option><option value="119(2)(b)">119(2)(b) — Condonation of delay</option></SelectField>
      ...
      {!dueDatePassed && filing.filingSection === '139(4)' && <div style={{ gridColumn: '1 / -1', padding: '8px 10px', borderRadius: 6, border: '1px solid #f0c36d', background: '#fdf6e3', fontSize: 12, color: '#7a5b00' }}>The {itrForm} due date for AY {assessmentYear} ({filingDueDate}) has not passed, so this return is not belated and cannot be filed under 139(4). Choose 139(1) while it is still on time, or 139(5) if you are revising a return that was already filed. Filing and validation are blocked until this is changed.</div>}
