 ...
 */
export function PersonalInfoTab({ formData, itrForm, onChange, onBanksChange }: PersonalInfoTabProps): React.JSX.Element {
  ...
  return <div>
    ...
    <SectionHeading title="Filing status and return history" description="The selected filing section activates the original-return and notice details required to prevent invalid return metadata." />
      <div style={CARD_STYLE}><div style={GRID_STYLE}>
        <SelectField label="Return filed under section" value={filingSection} onChange={(value) => patch({ filingSection: value, returnType: value === '139(5)' ? 'REVISED' : 'ORIGINAL' })} required><option value="139(1)">139(1) — On or before due date</option><option value="139(4)">139(4) — Belated return</option><option value="142(1)">142(1) — Notice response</option><option value="148">148 — Reassessment return</option><option value="153C">153C — Search-related return</option><option value="139(5)">139(5) — Revised return</option><option value="139(9)">139(9) — Defective-return response</option><option value="119(2)(b)">119(2)(b) — Condonation of delay</option></SelectField>
        ...
        <SelectField label="Tax Regime Election" value={text(formData.optOutNewTaxRegime) === 'Y' || formData.optOutNewTaxRegime === true ? 'OLD' : 'NEW'} onChange={(value) => patch({ optOutNewTaxRegime: value === 'OLD' ? 'Y' : 'N' })} required><option value="NEW">New tax regime</option><option value="OLD">Old tax regime / opt out</option></SelectField>
        {(text(formData.optOutNewTaxRegime) === 'Y' || formData.optOutNewTaxRegime === true) && <><Field label="Form 10-IEA Acknowledgement Number" value={text(formData.form10IEAAcknowledgementNumber)} onChange={(value) => patch({ form10IEAAcknowledgementNumber: value })} maxLength={50} /><Field label="Form 10-IEA Filing Date" value={text(formData.form10IEAFilingDate)} onChange={(value) => patch({ form10IEAFilingDate: value })} type="date" /></>}
      </div></div>
