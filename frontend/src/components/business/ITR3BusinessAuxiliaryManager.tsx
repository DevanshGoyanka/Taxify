...
const dcgFields = (keys: readonly string[]): Field[] => moneyFields(keys, keys);
...
const icUndertaking = (key: string, label: string): Group => ({ key, label, fields: [f('Sch80LocOrDescCode', 'State / undertaking code', 'text')], rows: { min: 1, fields: [f('DeductAmountSec80', 'Eligible deduction amount', 'readonly')] } });
const esrSections = ['Section35_1_i','Section35_1_ii','Section35_1_iia','Section35_1_iii','Section35_1_iv','Section35_2AA','Section35_2AB','Section35_CCC','Section35_CCD'] as const;
const esrFields = (): Field[] => [f('AmtDebPL', 'Expenditure debited to P&L'), f('AmtUs35Allowable', 'Eligible amount under section 35', 'readonly'), f('ExcessAmtOverDebPL', 'Excess over P&L debit', 'readonly')];
...
const esrGroups: Group[] = [...esrSections.map((key): Group => ({key,label:key,groups:[{key:'DeductUs35',label:'Scientific research expenditure',fields:esrFields()}]})), {key:'TotUs35',label:'Total section 35 claim',groups:[{key:'DeductUs35',label:'Computed total',fields:esrFields()}]}];
...
const specs: readonly ScheduleSpec[] = [
  ...
  { key:'ScheduleESR', label:'Schedule ESR — Expenditure on scientific research', groups:[{key:'DeductionUs35',label:'Deduction under section 35',groups:esrGroups}]},
  { key:'ScheduleUD', label:'Schedule UD — Unabsorbed depreciation', fields:[f('CurrAssYr','CurrAssYr','readonly'),...moneyFields(['CurBalCFNY','CurAllowBalCFNY','TotBFUDepritAmt','TotAdjustAccTax115BACAmt','TotCurYrdepritSetoffInc','TotDepritBalCFNY','TotBFUAllowAmt','TotCurYrAllowSetoffInc','TotalBalCFNY'],['TotBFUDepritAmt','TotAdjustAccTax115BACAmt','TotCurYrdepritSetoffInc','TotDepritBalCFNY','TotBFUAllowAmt','TotCurYrAllowSetoffInc','TotalBalCFNY'])],groups:[{key:'UnabsorbedDepreciation',label:'Assessment-year rows',rows:{fields:[f('AssYr','AssYr','text'),...moneyFields(['AmtBFUD','AdjustAccTax115BACAmt','AmtDeprSOCY','BalCFNY','AmtBFUAllow','AmtAllowSOCY','AllowBalCFNY'],['BalCFNY','AllowBalCFNY'])]}}]},
  { key:'ScheduleICDS', label:'Schedule ICDS — Ten standards', groups:[{key:'ICDSAdjustments',label:'ICDS adjustments',groups:['AccPolicyAmtDetl','InventoriesValueDetl','ConstContractsAmtDetl','RevenueRcgAmtDetl','TangibleFixedAssetDetl','ForeignExgRatesDetl','GovtGrantsDetl','SecuritiesDetl','BorrowingCostsDetl','ProvAssetsDetl','TotalNetAmtDetl'].map(key=>({key,label:key,fields:moneyFields(key==='TotalNetAmtDetl'?['IncreaseInProfit','DecreaseInProfit']:['IncreaseInProfit','DecreaseInProfit','NetEffect'],key==='TotalNetAmtDetl'?['IncreaseInProfit','DecreaseInProfit']:['NetEffect'])}))}]},
  ...
  { key:'Schedule80_IC', label:'Schedule 80-IC — North-East undertakings', fields:[f('Sch80SectionCode','Section code','readonly'),f('TotSchedule80_IC','Total Schedule 80-IC deduction','readonly')],groups:[{key:'DeductInNorthEast',label:'Eligible undertaking identity and deduction period',groups:[icUndertaking('Assam_Und','Assam'),icUndertaking('ArunachalPradesh_Und','Arunachal Pradesh'),icUndertaking('Manipur_Und','Manipur'),icUndertaking('Mizoram_Und','Mizoram'),icUndertaking('Meghalaya_Und','Meghalaya'),icUndertaking('Nagaland_Und','Nagaland'),icUndertaking('Tripura_Und','Tripura'),icUndertaking('Sikkim_Und','Sikkim')],fields:[f('TotDeductInNorthEast','Total North-East deduction','readonly')]}]},
  { key:'Schedule80RA', label:'Schedule 80RA — Research donations', fields:moneyFields(['TotalDonationAmtCash80RA','TotalDonationAmtOtherMode80RA','TotalDonationsUs80RA','TotalEligibleDonationAmt80RA'],['TotalDonationAmtCash80RA','TotalDonationAmtOtherMode80RA','TotalDonationsUs80RA','TotalEligibleDonationAmt80RA']),groups:[{key:'DonationDtlsRsrchAssctn',label:'DonationDtlsRsrchAssctn',rows:{fields:[f('NameOfDonee','NameOfDonee','text'),f('DoneePAN','DoneePAN','text'),f('DonationAmtCash'),f('DonationAmtOtherMode'),f('DonationAmt','DonationAmt','readonly'),f('EligibleDonationAmt')]},groups:[address]}]},
...
export default function ITR3BusinessAuxiliaryManager({ data, onChange, visibleSchedules, showHeading = true }: ITR3BusinessAuxiliaryManagerProps): React.JSX.Element {
  ...
  const update=(path:(string|number)[],value:ITR3AuxiliaryValue):void=>{const terminal=String(path[path.length-1]);if(['DeductAmountSec80','TotDeductInNorthEast','TotSchedule80_IC','Sch80SectionCode','ProfitShareAmt','TotalProfitShareAmt','TotalIntrstAmtDueOrRecv','TotalRemunernAmtDueOrRecv','TotalFirmCapBalOn31Mar'].includes(terminal))return; if(['IntrstAmtDueOrRecv','RemunernAmtDueOrRecv','FirmCapBalOn31Mar'].includes(terminal)&&typeof value==='number'&&(value<0||value>MAX))return; if(terminal==='ProfitSharePercent'&&typeof value==='number'&&(value<0||value>100))return; if(terminal==='FirmPAN'&&typeof value==='string'&&value!==''&&!/^[A-Z]{5}[0-9]{4}[A-Z]$/.test(value))return; const next=deepClone(draft);pathSet(next as Obj,path,value);const computed=recompute(next);setDraft(computed);onChange(computed);};
