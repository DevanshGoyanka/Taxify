...
| **Phase 3 — Type-3 Submission Automation** | ✅ IMPLEMENTED — AWAITING UAT | (pending approval) | Dedicated filing worker, deterministic JSON export, portal upload, e-verify, acknowledgement, and unified filing API. Existing Prefill/download worker is unchanged. **Phase 3 Addendum (2026-08-21):** R145 dividend-breakup correctness (Category B warning when no per-receipt data), detailed CBDT schema-violation logging in the 422 path, and a frontend **Direct Submit** button wired to the Type-3 portal upload flow. **Phase 3 compliance closure (2026-08-23):** §A7 standalone ack downloader (`app/eri/type3/ack_downloader.py`) + `POST .../acknowledgement/fetch` endpoint; §7.5/§10.1 `AuditLog` table + `app/services/audit_service.py` wired into generate/submit/upload/everify/ack; appendix dead-code removal of `app/services/submission_service.py`. |
| Phase 4 — Type-3 UAT Certification | ⏳ SUBMISSION 2 READY | `scripts/type3_uat_sanity.py` | §A11 UAT sanity pack generator implemented (ITR-1 ×2 + ITR-4 ×3 variants, real SW_ID + Digests verified against each written file's own bytes, `sanity_manifest.json`). **Submission 1 rejected 25-Aug-2026** on two counts: wrong SW_ID (`SW20014122`; the correct Type-3 UAT SW_ID is **`SW20014243`**, ERI_ID `ERIP013181`, secret + iteration `1646`, org "Sunit Ramashankar Goyanka"), and indentation whitespace in the JSON files. Both fixed — files are now minified per SOP §5.6 and every one verifies against its own bytes. Re-send and await ITD SW_ID enablement. |
